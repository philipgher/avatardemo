...
var faceColor = 0x878787;

THREE.JeelizHelper=(function(){
  //INTERNAL SETTINGS :
  var _settings={
    //THREE LIGHTS
    ambientLightIntensity: 0.7, // default = 1.0
    dirLightIntensity: 0.3, //fox : 1.2
  ...
  var _states={ //all possibles states (ENUM like)
    notLoaded:-10,
  ...
  var _state=_states.notLoaded; //initial state
  var _animationHandler=false;
  var _loading={
    modelURL: false,
    threeMat: false,
    callback: false,
  ...
  var _rotationAmortized=[0,0,0], _rotationSpeed=[0,0,0];
  var _prevT=0;
  ...
  function animate(t) {
    ...
    var amortizationCoeff=Math.pow(_settings.rotationAmortizationCoeff, dt/16);
  ...
  function change_material(mat){
    ...
    if (_state===_states.loading){
      var previousCallback=_loading.callback;
  ...
  function load_model(url, mat, callback){
    ...
    ThreeMorphAnimGeomBuilder({
      ...
      successCallback: function(geom){
        ...
        // mesh.add(wireframe);   // doenst work, wireframe is declared in ThreeMorphFlexibleMaterialBuilder, but not in this scope
        ...
        var morphTargetInfluencesDst=JEEFACETRANSFERAPI.get_morphTargetInfluencesStabilized();
    ...
    'init': function(spec){
      ...
      'set_materialParameters': function(params){
        ...
        var matParameters=Object.assign({
          'shininess': 10, // default was 30

          'color': faceColor, //default parameters
          'diffuse': faceColor,
          'specular': faceColor,
        ...
        var mat = ThreeMorphFlexibleMaterialBuilder(matParameters, _settings.rotationOrder);
        change_material(mat);
